...
void StreamCommandLineParser::parse(const QStringList &args, StreamingPreferences *preferences)
{
    ...
    parser.addToggleOption("vrr", "variable refresh rate streaming");
    ...
    if (parser.isSet("bitrate")) {
        ...
        if (!inRange(preferences->bitrateKbps, 500, 900000)) {
            fprintf(stderr, "Warning: Bitrate is out of the supported range (500 - 900000 Kbps). Performance may suffer!\n");
        }
